...
- (void)startShadowsocks: (ShadowsocksProxyCompletion)completion {
    ...
    [self _startShadowsocks];
}

- (void)_startShadowsocks {
    ServerProfileManager *profileMgr = [ServerProfileManager instance];
    ServerProfile *profile = [profileMgr getActiveProfile];
    if(profile == nil){
        NSLog(@"not active server");
        return;
    }
    NSString *host = profile.serverHost;
    NSNumber *port = [NSNumber numberWithInt:profile.serverPort ];
    NSString *password = profile.password;
    NSString *authscheme = profile.method;
...
- (void)onShadowsocksCallback:(int)fd {
    ...
    if (fd > 0) {
        ...
        error = [NSError errorWithDomain:@"com.cyc.ShadowsocksR" code:100 userInfo:@{NSLocalizedDescriptionKey: @"Fail to start http proxy"}];
    }
    if (self.shadowsocksCompletion) {
